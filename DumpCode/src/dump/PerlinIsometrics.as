	...
	public class PerlinIsometrics extends Sprite
	{
		...
		private function init():void
		{
			...
			for(var i:uint=0;i<wid;i++)
			{
				...
				{
					grid.addBox(new IsometricObject(i*10,0,j*10,10,0,10,false,0,true,0xffffff));
				}
			}
			
			for(i=grid.boxes.length;i>0;i--)
			{
				engine.drawBox(grid.boxes[i-1]);
			}
		...
		private function onClick(e:MouseEvent):void
		{
			...
			for(var i:uint=0;i<grid.boxes.length;i++)
			{
				grid.boxes[i].y = 0;
				grid.boxes[i].h = 0;
			}
		...
		private function onEnterFrame(e:Event):void
		{
			...
			var j:uint=0;
			var yVal:Number=0;
			var count:uint=grid.boxes.length;
			grid.graphics.clear();
			//engine.g = grid.graphics;
			if(float)
			{
				for(i=0;i<wid;i++)
				{
					for(j=0;j<hei;j++)
					{
						...
						grid.boxes[count-1].y = yVal;
						grid.boxes[count-1].fillColour = bmd.getPixel(i,j);
						engine.drawBox(grid.boxes[count-1]);
