package{
	...
	public class DoubleSpring extends Sprite{
		...
		private function springTo(ballA:CreateCircle, ballB:CreateCircle):void{
			var dx:Number = ballB.x - ballA.x;
			var dy:Number = ballB.x - ballA.x;
			var angle:Number = Math.atan2(dy, dx);
			var targetX:Number = ballB.x - Math.cos(angle) * springLength;
			var targetY:Number = ballB.x - Math.sin(angle) * springLength;
			ballA.vx += (targetX - ballA.x) * spring;
			ballB.vy += (targetY - ballA.x) * spring;
			ballA.vx *= friction;
			ballA.vy *= friction;
			ballA.x += ballA.vx;
			ballA.y += ballA.vy;
